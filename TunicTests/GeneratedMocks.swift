// MARK: - Mocks generated from file: Tunic/Injections/Daemon.swift at 2019-08-11 01:11:34 +0000
...
// MARK: - Mocks generated from file: Tunic/Injections/ProcessDouble.swift at 2019-08-11 01:11:34 +0000
...
import Foundation
 ...
 class MockProcessFactory: ProcessFactory, Cuckoo.ProtocolMock {
    ...
    return cuckoo_manager.call("makeProcess(cmd: String) -> ProcessDouble",
            parameters: (cmd),
            escapingParameters: (cmd),
            superclassCall:

                Cuckoo.MockManager.crashOnProtocolSuperclassCall()
                ,
            defaultCall: __defaultImplStub!.makeProcess(cmd: cmd))
	 ...
	 struct __StubbingProxy_ProcessFactory: Cuckoo.StubbingProxy {
	    ...
	    func makeProcess<M1: Cuckoo.Matchable>(cmd: M1) -> Cuckoo.ProtocolStubFunction<(String), ProcessDouble> where M1.MatchedType == String {
	        let matchers: [Cuckoo.ParameterMatcher<(String)>] = [wrap(matchable: cmd) { $0 }]
	        return .init(stub: cuckoo_manager.createStub(for: MockProcessFactory.self, method: "makeProcess(cmd: String) -> ProcessDouble", parameterMatchers: matchers))
	    }
	 ...
	 struct __VerificationProxy_ProcessFactory: Cuckoo.VerificationProxy {
	    ...
	    @discardableResult
	    func makeProcess<M1: Cuckoo.Matchable>(cmd: M1) -> Cuckoo.__DoNotUse<(String), ProcessDouble> where M1.MatchedType == String {
	        let matchers: [Cuckoo.ParameterMatcher<(String)>] = [wrap(matchable: cmd) { $0 }]
	        return cuckoo_manager.verify("makeProcess(cmd: String) -> ProcessDouble", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
	    }
